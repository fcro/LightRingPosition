...
function envoiCoord() {
	$.ajax({
		...
		url : "webapi/jeu/coord/"+Math.floor(x/ (canvas_height/taille_plateau))+"/"+Math.floor(y/ (canvas_width/taille_plateau))+"/envoi",
		dataType : "json",
		data : JSON.stringify({
		}),
...
function verifCavalier() {
	$.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : "webapi/jeu/coord/"+Math.floor(x/ (canvas_height/taille_plateau))+"/"+Math.floor(y/ (canvas_width/taille_plateau))+"/verif",
		dataType : "json",
		data : JSON.stringify({
			"x" : Math.floor(x/ (canvas_height/taille_plateau)),
			"y" : Math.floor(y/ (canvas_width/taille_plateau))
		}),
		success : function(data, textStatus, jqXHR) {
			console.log("success");
